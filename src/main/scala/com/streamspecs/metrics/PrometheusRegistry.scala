...
/** Owns Prometheus collectors and maps StreamSpecs dotted keys onto labeled counters. */
final class PrometheusRegistry(val collectorRegistry: CollectorRegistry):
  ...
  private val ruleViolations: Counter =
    Counter
      ...
      .help("Stateless / rule-based validation failures")
  ...
  /** Fallback counters for unrecognized keys (sanitized metric name). */
...
object PrometheusRegistry:
  ...
  def startHttpServer(
    ...
    new HTTPServer(
      ...
      true // daemon thread so JVM can exit cleanly with Resource release
    )
end PrometheusRegistry
