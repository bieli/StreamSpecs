...
/** CLI / runtime overrides for metrics HTTP scrape server and backend. */
...
object CliOptions:
  ...
  /** Parse argv. Unknown flags are reported as errors. */
        ...
        case "--metrics-port" :: value :: tail =>
          value.toIntOption match
            ...
            case _ =>
              loop(
                tail,
                acc.copy(errors = acc.errors :+ s"Invalid --metrics-port value: '$value'")
              )
  ...
  def helpText: String =
    """StreamSpecs - streaming data-quality validator
      ...
      |  --metrics-server[=true|false]  Enable/disable HTTP /metrics server (default: from config)
      ...
      |
      |Also supported via environment (overrides application.conf, overridden by CLI):
      |  STREAMSPECS_METRICS_SERVER=true|false
      |  STREAMSPECS_METRICS_PORT=9464
      |  STREAMSPECS_METRICS_HOST=0.0.0.0
      |  STREAMSPECS_METRICS_BACKEND=prometheus
      |
      |Precedence: CLI flags > environment > application.conf
      |
      |Examples:
      |  sbt "run -- --no-metrics-server"
      |  sbt "run -- --metrics-server --metrics-port 9100"
      |  STREAMSPECS_METRICS_SERVER=false sbt run
      |""".stripMargin
  ...
  /** Apply CLI overrides on top of config already loaded (HOCON + env). */
  def applyTo(config: StreamValidatorConfig, cli: CliOptions): StreamValidatorConfig =
