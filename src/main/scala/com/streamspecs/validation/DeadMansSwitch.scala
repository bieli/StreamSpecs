...
/** Shared last-seen timestamp for the Dead Man's Switch. */
...
object DeadMansSwitch:
  ...
  /** Background FS2 stream that emits [[StatefulAlert.TemporalAnomaly]] when no events have been
    * observed for longer than `rule.maxIdleDuration`.
    *
    * Edge-triggered: one alert per silence gap; resets after traffic resumes.
    */
  def watchdog(
